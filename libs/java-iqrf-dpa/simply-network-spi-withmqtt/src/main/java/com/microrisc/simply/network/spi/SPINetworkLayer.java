...
public final class SPINetworkLayer extends AbstractNetworkLayer {
    ...
    private MQTTCommunicator mqttCommunicator = null;
    private String protocol = "tcp://";
    private String broker = "localhost";
    private int port = 1883;
    private String clientId = "b827eb26c73d-spi";
        ...
        public void run() {
            while ( true ) {
                ...
                while ( !consumedData.isEmpty() ) {
                    final short[] userData = consumedData.poll();

                    Thread messageProcessorThread = new Thread() {
                        public void run() {
                            sendIncomingMessage(userData);
                        }
                    };
     ...
     */
    public SPINetworkLayer(NetworkConnectionStorage connectionStorage, String portName) throws MqttException {
        ...
        this.mqttCommunicator = new MQTTCommunicator(url, clientId, cleanSession, quietMode, userName, password, "STD");
    }
    ...
    public void sendData(NetworkData networkData) throws NetworkLayerException {
        ...
        final short[] userData = networkData.getData();
        Thread messageProcessorThread = new Thread() {
            public void run() {
                sendIncomingMessage(userData);
            }
    ...
    private void sendIncomingMessage(short[] userData) {

        StringBuilder buffer = new StringBuilder();
        for (int index = 0; index < userData.length; index++) {
            buffer.append(String.format("%02X", userData[index]));
        }

        // web log
        try {
            mqttCommunicator.publish(MQTTTopics.STD_DPA_PACKETS, 2, buffer.toString().getBytes());
        } catch (MqttException ex) {
            logger.error("Data has not been sent to mqtt! " + ex.getMessage());
        }
    }
}
